...
public abstract class AbstractDwelling {
	...
	public AbstractDwelling(double areaInSquareMeters, double priceInUSD, Location location) {
	...
	public double calculateDistanceToCloserSchoolInMeters(List<Location> locationsOfSchools) {
		...
		for (Location locationOfSchool : locationsOfSchools) {
			distance = Math
					.sqrt(Math.pow((locationOfSchool.getXInDecimalDegrees() - location.getXInDecimalDegrees()), 2)
	...
	public double calculateDistanceToCloserKindergardenInMeters(List<Location> locationsOfKindergardens) {
		...
		for (Location locationOfKindergarden : locationsOfKindergardens) {
			distance = Math.sqrt(Math
					.pow((locationOfKindergarden.getXInDecimalDegrees() - location.getXInDecimalDegrees()), 2)
					+ Math.pow((locationOfKindergarden.getYInDecimalDegrees() - location.getYInDecimalDegrees()), 2));
			if (distance < smallestDistance)
				smallestDistance = distance;
	...
	public double calculateDistanceToCloserPlaygroundInMeters(List<Location> locationsOfPlaygrounds) {
		...
		double smallestDistance = Math.sqrt(Math
				...
				+ Math.pow((locationsOfPlaygrounds.get(0).getYInDecimalDegrees() - location.getYInDecimalDegrees()),
						2));

		for (Location locationOfPlayground : locationsOfPlaygrounds) {
			distance = Math.sqrt(
					Math.pow((locationOfPlayground.getXInDecimalDegrees() - location.getXInDecimalDegrees()), 2) + Math
							.pow((locationOfPlayground.getYInDecimalDegrees() - location.getYInDecimalDegrees()), 2));
			if (distance < smallestDistance)
				smallestDistance = distance;
